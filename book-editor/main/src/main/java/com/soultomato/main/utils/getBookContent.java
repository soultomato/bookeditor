...
public class getBookContent {
    ...
    public static Path findFileInDirectory(String directoryPath, String fileNameToFind) throws IOException {
        Path dir = Paths.get(directoryPath);
        try (DirectoryStream<Path> stream = Files.newDirectoryStream(dir)) {
            for (Path entry : stream) {
                if (Files.isDirectory(entry)) {
                    Path foundFile = findFileInDirectory(entry.toString(), fileNameToFind);
                    if (foundFile != null) {
                        return foundFile;
                    }
                } else if (entry.getFileName().toString().equals(fileNameToFind)) {
                    return entry;
                }
            }
        }
